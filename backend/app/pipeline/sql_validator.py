...
class SQLValidator:
    ...
    def validate_sql(self, sql_query: str, validated_plan: dict) -> tuple[bool, str]:
        ...
        # Fix SQL Server LIMIT N syntax if generated by LLM
        import re
        match_limit = re.search(r'\bLIMIT\s+(\d+)\b', clean_sql, re.IGNORECASE)
        if match_limit:
            limit_num = match_limit.group(1)
            clean_sql = re.sub(r'\bLIMIT\s+\d+\b', '', clean_sql, flags=re.IGNORECASE).strip()
